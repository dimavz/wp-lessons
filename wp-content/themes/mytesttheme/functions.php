...
{
    // Включаем поддержку темой многоязычности
...
// далее в цикле WP loop выводим этот размер так: the_post_thumbnail( 'spec_thumb' );
    ...
    register_nav_menus(array(
        'header_menu' => __('Выводится в шапке', 'mytesttheme'),
        'footer_menu' => __('Выводится в футере', 'mytesttheme'),
    ));
...
//    register_sidebars( 2, $args ); // Регистрирует несколько панелей виджетов (место в админке куда виджеты будут размещаться)
    ...
    $args2 = array(
        ...
        'description' => __('Область для виджетов в левом сайдбаре', 'mytesttheme'),
        'before_widget' => '',
        'after_widget' => "\n",
    );
    ...
    $args3 = array(
        ...
        'description' => __('Область для виджетов в панели поиска', 'mytesttheme'),
        'before_widget' => '',
        'after_widget' => "\n",
    );
...
function mytesttheme_filter_search_form($form, $args)
{
    ...
    $search = __('Поиск', 'mytesttheme');
    $form = <<<FORM
    <form class="form-inline my-2 my-lg-0 searchform" role="search" method="get" id="searchform" action="">
        <input class="form-control mr-sm-2" type="text" placeholder="Search" aria-label="Search" name="s">
        <button id="searchsubmit" class="btn btn-outline-success my-2 my-sm-0" type="submit">
            $search
        </button>
    </form>
FORM;
